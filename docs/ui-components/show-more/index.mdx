...
<Exercise title="演習-発展1（コンテンツの最後に「もっと見る」ボタンを配置）">

以下のプレビューと同じように、「もっと見る」ボタンをコンテンツの外ではなく、コンテンツの下部（フェードの上）に配置してください。

- ボタンはコンテンツの下部に固定配置
- 開いたときにボタンがコンテンツの最後に移動する
...
<CodePreview
  ...
  .content {
    ...
    overflow: hidden;
    transition: max-height 0.5s;
    padding-bottom: 40px; /* ボタンの高さ分の余白 */
  }
...
<Exercise title="演習-発展2（段階的に開く「もっと見る」）">
