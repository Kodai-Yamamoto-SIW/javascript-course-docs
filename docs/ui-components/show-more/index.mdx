...
<CodePreview
  ...
  jsVisible={false}
  previewVisible={true}
  initialHTML={`<div class="wrap"> <!-- コンテンツとフェードをまとめるラッパー -->
    <p class="content">
      Web開発の基礎を学びましょう。<br>
      HTMLは構造を作り、CSSは見た目を整え、JavaScriptは動きを加えます。<br>
      これらを組み合わせることで、魅力的なWebサイトが作れます。<br>
...
<CodePreview
  ...
  initialJS={`let moreYoso = document.querySelector('.more'); // ボタンを取得
...
<Exercise title="演習-発展1（画像ギャラリーの「もっと見る」）">

以下のプレビューと同じように、画像ギャラリーで「もっと見る」を実装してください。

- 最初は3枚の画像だけ表示
- 「もっと見る」ボタンをクリックすると、残りの画像も表示される
- 「閉じる」ボタンをクリックすると、最初の3枚だけに戻る
...
<CodePreview
  ...
  initialHTML={`<div class="gallery">
    <div class="image-item">画像 1</div>
    <div class="image-item">画像 2</div>
    <div class="image-item">画像 3</div>
    <div class="image-item hidden">画像 4</div>
    <div class="image-item hidden">画像 5</div>
    <div class="image-item hidden">画像 6</div>
  </div>
  <button class="more">もっと見る</button>`}
  initialCSS={`.gallery {
    display: grid;
  ...
  initialJS={`let moreYoso = document.querySelector('.more');
  let hiddenImages = document.querySelectorAll('.image-item.hidden'); // 非表示の画像を全て取得
...
<Exercise title="演習-発展2（横スクロールの「もっと見る」）">

以下のプレビューと同じように、横スクロール可能なカードリストで「もっと見る」を実装してください。

- 最初は一部だけ見えている状態
- 「もっと見る」ボタンをクリックすると、横スクロールできるようになる
...
<CodePreview
  ...
  jsVisible={false}
  previewVisible={true}
  initialHTML={`<div class="scroll-wrap">
    <div class="card-list">
  ...
  .card-list {
    display: flex;
    gap: 16px;
    overflow-x: hidden; /* 最初は横スクロールを隠す */
    transition: overflow-x 0.3s;
  }

  .scroll-wrap.open .card-list {
    overflow-x: auto; /* 開いたら横スクロール可能にする */
  }
  ...
  moreYoso.addEventListener('click', function() {
    scrollWrap.classList.toggle('open');
...
<Exercise title="演習-発展3（上下両方向から開く「もっと見る」）">

以下のプレビューと同じように、上下にフェードがあり、中央部分だけが見えている状態から開く「もっと見る」を実装してください。

- 上下にフェードを配置
- 「もっと見る」ボタンをクリックすると、上下両方向に広がる
...
<CodePreview
  ...
  .content {
    ...
    overflow: hidden;
    transition: max-height 0.5s;
    padding-top: 50px; /* 上部フェードの高さ分の余白 */
  }

  .wrap.open .content {
    max-height: 600px;
    padding-top: 0; /* 開いたら余白を消す */
  }
...
<Exercise title="演習-発展4（段階的に開く「もっと見る」）">
...
<CodePreview
  ...
  moreYoso.addEventListener('click', function() {
    ...
    // クラスをリセット
    wrapperYoso.classList.remove('stage1', 'stage2');
